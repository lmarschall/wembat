...
describe("decrypt", () => {
    ...
    it("gibt entschlüsseltes Objekt zurück, wenn alles korrekt ist", async () => {
        // Mocks
        deriveEncryptionKey.mockResolvedValue("mockEncryptionKey");
        str2ab.mockReturnValue(new ArrayBuffer(8));
        globalThis.window.crypto.subtle.decrypt.mockResolvedValue(new TextEncoder().encode("Decrypted Message"));
    ...
    it("gibt Fehler zurück, wenn decrypt fehlschlägt", async () => {
        deriveEncryptionKey.mockResolvedValue("mockEncryptionKey");
        str2ab.mockReturnValue(new ArrayBuffer(8));
        globalThis.window.crypto.subtle.decrypt.mockRejectedValue(new Error("Decrypt failed"));
        const mockKey = {};
