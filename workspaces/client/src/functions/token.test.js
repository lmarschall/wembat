...
describe("token", () => {
    ...
    it("should return error if jwtString is undefined", async () => {
        const result = await token(mockAxiosClient, undefined);
    ...
    it("should return the same token if it is not expired", async () => {
        ...
        const result = await token(mockAxiosClient, validJWT);
    ...
    it("should refresh token if expired and axiosClient returns status 200", async () => {
        ...
        mockAxiosClient.post.mockResolvedValue(axiosResponse);
        const result = await token(mockAxiosClient, expiredJWT);
        expect(mockAxiosClient.post).toHaveBeenCalledWith(`/refresh-token`, {
            userInfo: {
                userMail: "test@mail.com",
                sessionId: "session123",
            },
