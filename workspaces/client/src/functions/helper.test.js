...
import { str2ab, ab2str, jwtDecode, bufferToArrayBuffer, deriveEncryptionKey, loadCryptoPublicKeyFromString, loadCryptoPrivateKeyFromString, } from "./helper";
...
describe("helper functions", () => {
    ...
    describe("deriveEncryptionKey", () => {
        it("should derive an encryption key when keys are provided", async () => {
            const derivedKey = await deriveEncryptionKey(dummyCryptoKey, dummyCryptoKey);
        ...
        it("should throw an error if one of the keys is undefined", async () => {
            await expect(deriveEncryptionKey(undefined, dummyCryptoKey)).rejects.toThrow("Could not derive Encryption Key");
        });
    });
    ...
    describe("loadCryptoPublicKeyFromString", () => {
        it("should load a public CryptoKey from a string", async () => {
            ...
            expect(window.crypto.subtle.importKey).toHaveBeenCalledWith("jwk", dummyJWK, { name: "ECDH", namedCurve: "P-384" }, true, []);
        });
    ...
    describe("loadCryptoPrivateKeyFromString", () => {
        it("should load a private CryptoKey from a string", async () => {
            ...
            const importedKey = await loadCryptoPrivateKeyFromString(privKeyString);
            expect(importedKey).toBe(dummyCryptoKey);
            expect(window.crypto.subtle.importKey).toHaveBeenCalledWith("jwk", dummyJWK, { name: "ECDH", namedCurve: "P-384" }, true, ["deriveKey", "deriveBits"]);
        });
        it("should throw an error if the private key string is empty", async () => {
            await expect(loadCryptoPrivateKeyFromString("")).rejects.toThrow("Private Key String empty");
        });
    });
});
