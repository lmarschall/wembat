...
class WembatSession {
    ...
    // main function
    async register(challengeOptions, token = "") {
        console.log(challengeOptions);
        const credentials = await startRegistration(challengeOptions).catch((err) => {
            throw Error(err);
        });
    ...
    // main function
    async login(challengeOptions, credentials) {
    ...
    // main function
    async loginRead(challengeOptions) {
        let privKey;
        console.log(challengeOptions);
        ...
        if (Object.keys(credentials.clientExtensionResults.largeBlob).length) {
            ...
            privKey = await window.crypto.subtle.importKey("jwk", 
    ...
    // main function
    async loginWrite(challengeOptions) {
        console.log(challengeOptions);
